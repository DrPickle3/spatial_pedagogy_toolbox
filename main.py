...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        default=500,
        help="The width and height for the main image canvases.",
    )
    ...
    parser.add_argument(
        "--experiment_name", type=str, help="The name for the experiment."
    )
    parser.add_argument(
        "--force-overwrite",
        action="store_true",
    ...
    if not experiment_name:
        experiment_name = simpledialog.askstring(
            "Experiment Name", "Enter a name for this experiment:"
        )
    ...
    if not experiment_name:
        ...
        if (
            ...
            and os.listdir(experiment_path)
            and not args.force_overwrite
        ):
            print(
                f"Error: Experiment folder '{experiment_path}' is not empty. Use --force-overwrite to proceed.",
                file=sys.stderr,
            )
            root.destroy()
            sys.exit(1)
